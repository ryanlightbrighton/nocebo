...
if (_proceed) then {
	...
	private _actionItems = ["chopwood",_assignedItem] call horde_fnc_spawnActionItems;
	...
	if (_proceed) then {
		// get size of tree/bush to determine output
		private _volume = boundingBoxReal _object select 1;
		_volume = _volume vectorMultiply 2;
		_volume params ["_xV","_yV","_zV"];
		_volume = _xV * _yV * _zV;
		private _objPos = ASLtoAGL getPosASL _object;
		_object setDamage 1;
		private _holder = (getPosASL _object) call horde_fnc_returnNearestHolder;
		if (isNull _holder) then {
			private _spawnPos = _objPos vectorAdd [0,0,0.05];
			_holder = spawnVeh("WeaponHolderSimulated",_spawnPos);
		};
		getArray (_cfgAction >> "output") params ["_item"];
		_holder addItemCargoGlobal [_item,(floor random (_volume * 0.001)) + 1];
		private _text = "
