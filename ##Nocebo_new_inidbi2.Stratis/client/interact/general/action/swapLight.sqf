...
if (_proceed) then {
	private _light = getVarDef(player,"playerHasAttachedAmmoObject",objNull);
	player playActionNow "PutDown";
	detach _light;
	_object attachTo [player,[0,-0.07,0.06],"LeftShoulder"];
	player setVariable ["playerHasAttachedAmmoObject",_object];
} else {
	private _text = "
