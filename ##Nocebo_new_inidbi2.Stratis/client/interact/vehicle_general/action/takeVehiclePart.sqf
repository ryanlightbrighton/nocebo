...
/*if (_currHealth > 10) then {*/
	...
	if same(_partPrefix,"ItemTyre_") then {
	...
	if (_skillName == "skillEngineer") then {
		...
		_actionItems = ["none",_item] call horde_fnc_spawnActionItems;
	};
	...
	private _generators = nearestObjects [_object, ["ncb_obj_portable_generator"],12];
	...
	if (isNull _display or {time < _timeOut}) then {
		...
		private _partSuffix = [typeOf _object,configName _cfgEntry,_hitPointDamage] call horde_fnc_realDamToRepairHealth;
		_partSuffix = 10 * (floor (_partSuffix / 10));
		/*if part suddenly damaged/destroyed then abort*/
		/*if (_partSuffix >= 10) then {*/
			private _bagClass = _partPrefix + (str _partSuffix);
			private _damage = [typeOf _object,configName _cfgEntry,0] call horde_fnc_repairHealthToRealDam;
			{
				private _hp = toLower _x;
				if ({_hp find _x > -1} count ["turret","gun","missile"] > 0) then {
					[_object,[_x,1]] call horde_fnc_setHitPointDamageGlobal;
				} else {
			...
			if not empty(_animArr) then {
				_animArr params ["_animType","_animPartsArr"];
				{
					...
					if (diff(_phase,1) and {_damage >= _threshold}) then {
						if same(_animType,"animate") then {
			...
			private _suffix = "";
			if (_partSuffix >= 10) then {
				_suffix = " and put it on your back";
				if same(backpack player,"") then {
					player addBackpack _bagClass;
				} else {
					_suffix = " and placed it on the ground";
					private _objectPosASL = ATLtoASL (player modelToWorld [0,1.5,0]);
					private _holder = _objectPosASL call horde_fnc_returnNearestHolder;
					if (isNull _holder) then {
						private _holderSpawnPos = player modelToWorld [1.1,-0.1,1];
						_holder = spawnVeh("WeaponHolderSimulated",_holderSpawnPos);
						_holder disableCollisionWith player;
						_holder disableCollisionWith _object;
					};
					_holder addBackpackCargoGlobal [_bagClass,1];
				};
			} else {
				_suffix = "but it was too badly damaged";
			};
			...
			private _failText = "You took the %1 but broke the %2";
			private _repairText = "You took the %1" + _suffix;
