...
horde_fnc_playerJoinGroup = {
	...
	if (not isNull _groupNew
	    ...
	    and {(_groupNew getVariable ["ncb_groupBans",[]]) find player == -1}
	) then {
...
horde_fnc_playerSendInvite = {
	...
	if (not isNull _player and {not (group player isEqualTo group _player)} and {player isEqualTo leader group player}) then {
		...
		_bans deleteAt (_bans find _player);
		_bans = _bans - [objNull];
...
horde_fnc_squadActionSelected = {
	...
	call {
		...
		if (_action == "promote unit") exitWith {
			[group player,_player] remoteExecCall [
				"selectLeader",
				group player
			];
			private _text = format [
...
_privateStatusCheckBoxCtrl ctrlSetEventHandler [
	"CheckedChanged","
		if (_this select 1 == 1) then {
			group player setVariable ['ncb_privateGroup',true]
		} else {
			group player setVariable ['ncb_privateGroup',false]
		};
		false
	"
];
...
_squadActionListBoxCtrl ctrlSetEventHandler [
	"LBSelChanged","
		diag_log format ['_squadActionListBoxCtrl %1',_this];
...
// NOTE:  NEED TO CONVERT BAN SYSTEM OVER TO UID!!!

0 spawn {
	scriptName "groupMenu";
		...
		call {
			...
			if (_text == "players") exitWith {
				...
				};
				{
					...
					if (_x in (group player getVariable ["ncb_groupBans",[]])) then {
