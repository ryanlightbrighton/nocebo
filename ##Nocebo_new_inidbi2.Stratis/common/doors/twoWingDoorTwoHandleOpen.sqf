...
params ["_house","_selectionName","_doorType","_doorA","_handleA","_doorB","_handleB"];
...
if not (isNull _sesame) then {
	...
	_house animateSource [_handleA, 1];
	_house animateSource [_doorB, 1];
	_house animateSource [_handleB, 1];
	if not (isPlayer _sesame) then {
		[
			[_this],{
		        call horde_fnc_getEachFrameArgs params ["_doorInfo"];
		        _doorInfo call horde_fnc_twoWingDoorTwoHandleClose;
