...
sleep 0.25;
/*inGameUISetEventHandler ["Action", "diag_log _this;false"];*/
...
(_veh turretUnit [1]) addAction ["Open group menu", "closeDialog 0; createDialog 'ncb_dlg_dynamicGroups'"];
(_veh turretUnit [1]) addAction ["View Dist: 1500m", "setViewDistance 1500;setObjectViewDistance [1500,ncb_param_shadowRenderDistance]"];
(_veh turretUnit [1]) addAction ["View Dist: 2000m", "setViewDistance 2000;setObjectViewDistance [2000,ncb_param_shadowRenderDistance]"];
(_veh turretUnit [1]) addAction ["View Dist: 3000m", "setViewDistance 3000;setObjectViewDistance [3000,ncb_param_shadowRenderDistance]"];
(_veh turretUnit [1]) addAction ["View Dist: 4000m", "setViewDistance 4000;setObjectViewDistance [4000,ncb_param_shadowRenderDistance]"];
player remoteControl (_veh turretUnit [1]);
_veh switchCamera "gunner";
sleep 0.75;
ncb_layer_staticScreen cutText ["","PLAIN DOWN"];
ncb_gv_casPlyrGrpActnRemCtrl = [_veh,_grp]; // used for group menu
waitUntil {
	not alive _veh
	...
	or {not alive (_veh turretUnit [1])}
};
...
objNull remoteControl (_veh turretUnit [1]);
