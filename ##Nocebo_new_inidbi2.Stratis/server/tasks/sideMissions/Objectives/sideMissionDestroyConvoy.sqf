...
scriptName "sideMissionDestroyConvoy";
...
if not empty(ncb_gv_zonesWithEmptySpace) then {
								...
								for "_l" from 0 to count _locations - 1 do {
									...
									if not ([getPos (_startRoads select 0),_position,500] call horde_fnc_waterBetweenPoints) then {
										...
										if not ([] isEqualTo _endRoads) exitWith {
											...
											_convoyGrp setGroupIDGlobal [format ["sm_destroyConvoy_%1",_convoyGrp]];
	...
	if not (isNull _convoyGrp) then {
		...
		_statement = format ["convoy en-route from %1 to %2.  Stop the convoy and kill all the occupants.  They may have important documents on the vehicles so be sure to search them if you can.",_startLocName,_endLocName];
		private _taskID = [
			"DC" + (str _startZone) + (str _endZone), // params
			true, // target
			[_statement,format ["Destroy Convoy %1",_convoyName],"convoyMkr"], // desc
			objNull, // no marker
			"CREATED", // state
			0, // priority
		...
		hint parsetext "<t align='center'><t size='2.2'>New Side Mission</t><br/><t size='1.5' color='#00B2EE'>Destroy Convoy</t><br/>____________________<br/>There is an ememy convoy on the island possibly containing useful materiel.  Destroy it to liberate the useful equipment.</t>";

		// conds

		[_vehicles,_convoyGrp,_endRoad,_taskID,_convoyName,round time + 900] spawn {
...

			{
				...
				_x setVariable ["ncb_wheels",_wheels]
			} count _vehicles;
			waitUntil {
				sleep 5;
				...
				if not (_allDead) then {
					{
						...
						if (alive _veh
							...
							} count (_veh getVariable ["ncb_wheels",[]])
						};
						true
					} count _vehicles;
				};
