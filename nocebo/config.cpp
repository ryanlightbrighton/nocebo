...
/*
	diag_mergeConfigFile ["P:\nocebo\config.cpp"];
	diag_mergeConfigFile ["E:\Documents\ArmaMods\nocebo\config.cpp"];

	https://community.bistudio.com/wiki/diag_resetShapes

	INDIVIDUAL (PROB NOT NEEDED)
	diag_mergeConfigFile ["P:\nocebo\hpp\ammo.hpp"];
	diag_mergeConfigFile ["P:\nocebo\hpp\cloudlets.hpp"];
	diag_mergeConfigFile ["P:\nocebo\hpp\headgear.hpp"];
	diag_mergeConfigFile ["P:\nocebo\hpp\loot.hpp"];
	diag_mergeConfigFile ["P:\nocebo\hpp\magazines.hpp"];
	diag_mergeConfigFile ["P:\nocebo\hpp\sounds.hpp"];
	diag_mergeConfigFile ["P:\nocebo\hpp\vehicles.hpp"];
	diag_mergeConfigFile ["P:\nocebo\hpp\weapons.hpp"];
	diag_mergeConfigFile ["P:\nocebo\hpp\ui.hpp"];

	// generate cfgPatches (run in vanilla only)
...
class CfgPatches {
	class Nocebo {
		...
		units[] = {"ncb_player_1","ncb_player_2","ncb_player_3","Horde_gunmanUnit","Horde_diverUnit","Horde_paraUnit","Horde_sniperUnit","Horde_renegadeUnit","Horde_dummy_unit"};
		weapons[] = {};
	};
};
...
class CfgLights {
	class ncb_fireLight {
        ...
        ambient[] = {1};
        brightness = 10;
        intensity = "4500*(power interpolate[0.1, 1.0, 0.4, 1.0])";
        blinking = 0;
        drawLight = 0;
        class Attenuation {
            start = 1;
...
// gets rid of buttons on main menu
...
class CfgLocationTypes {
	...
	class horde_ammo_dump : horde_zone {
		name = "Ammo dump";
	};
	class horde_fuel_dump : horde_zone {
		name = "Fuel dump";
	};
};
...
class CfgNonAiVehicles {
	class Eagle_F;
	class Horde_Eagle_F : Eagle_F {
		canBeShot = 1;
		minHeight = 5;  // Min flight altitude
		avgHeight = 50;  // Average height of flight
		maxHeight = 100;  // Max altitude flight
	};
	class Seagull;
	class Horde_Seagull_F : Seagull {
		canBeShot = 1;
		//singSound[] = {"A3\Sounds_F\ambient\animals\birds1",0.891251,1,200};
	};
};
...
#include "hpp\interactionInfo.hpp"
#include "defines\rscDefines.hpp"
#include "hpp\ammo.hpp"
#include "hpp\anim.hpp"
#include "hpp\cloudlets.hpp"
#include "hpp\functions.hpp"
#include "hpp\gear.hpp"
#include "hpp\magazines.hpp"
#include "hpp\map.hpp"
#include "hpp\sounds.hpp"
#include "hpp\ui.hpp"
#include "hpp\vehicles.hpp"
#include "hpp\weapons.hpp"
#include "gui\displayTitles.hpp"
#include "hpp\compressFunctions.hpp"
#include "hpp\worlds.hpp"
// #include "hpp\difficulty.hpp"

/*
class CfgFSMs {
	class Formation;
	class Horde_Formation : Formation {};
};
class CfgFormations {
	class West {
		class Wedge {
...
class RscDisplayMain: RscStandardDisplay {
	class Spotlight {
		class CoopCampaign {
			text = "Nocebo"; // Text displayed on the square button, converted to upper-case
...
class CfgMissions {
	class Cutscenes {
		class nocebo_intro_1 {
			directory = "nocebo\intro\nocebo_intro_1.Stratis";
		};
	};
};
*/
