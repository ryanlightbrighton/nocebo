...
with uiNamespace do {
	_fnc_compress = {
				...
				if (34 isEqualTo _x) then {
					...
					call {
						...
						if (_x isEqualTo 111
						    ...
						    and {not(_array select idx(2) in _alphabet)}
						) exitWith {
							_end = idx(1);
						...
						if (_x isEqualTo 97
						    ...
						    and {not(_array select idx(-1) in _alphabet)}
						    and {_array select idx(1) isEqualTo 110}
						    and {_array select idx(2) isEqualTo 100}
						    and {not(_array select idx(3) in _alphabet)}
						) exitWith {
							_end = idx(2);
						...
						if (_x isEqualTo 110
						    and {_forEachIndex < _maxIndex - 3}
						    and {not(_array select idx(-1) in _alphabet)}
						    and {_array select idx(1) isEqualTo 111}
						    and {_array select idx(2) isEqualTo 116}
						    and {not(_array select idx(3) in _alphabet)}
						) exitWith {
							_end = idx(2);
		...
		// now we have the indexes for true strings and also variable names defined in string (also, everything else apart from true strings is converted to lower case)

		// remove whitespace now and then look for private _varNames after
				...
				if not (_forEachIndex in _stringCharIndexes) then {
					...
					if (_x in _wsChars) then {
						...
						for "_i" from _forEachIndex - 1 to 0 step - 1 do {
							call {
								if (_array select _i in _alphabet) exitWith {
									_lastChar = "alphabet";
									_start = _i + 1;
									breakTo "mercury"
								};
		...
		private _indexCounter = [0,0];
		{
			...
			if (count _variable > 2) then {
				...
				if (_indexCounter select 0 == _alphaMaxIndex) then {
					_indexCounter set [0,0];
				};
				_indexCounter = [(_indexCounter select 0) + 1,0];
			};
	...
	private _functions = [];
	{
		_x params ["_cfgName","_public"];
		{
			...
			/*_public = true;*/
			/*_fncName = "horde_fnc_addEachFrame";*/
			diag_log format ["%1",_fncName];
			_text = (preprocessFile (getText (_x >> "file"))) call _fnc_compress;
			/*_text = (preprocessFile "nocebo\compressedFunctions\common\eachframe\addEachFrame.sqf") call fnc_compress;*/
			diag_log "------------------";
			/*uiNameSpace setVariable [_fncName,compileFinal _text];*/
			uiNameSpace setVariable [_fncName,compile _text];
